...
class HoursEntry(db.Model):
    ...
    def revenue_generated(self):
        """Revenue generated by this entry = hours_billed * hourly_rate"""
        if not self.employee:
            return 0.0
        return self.hours_billed * (self.employee.hourly_rate or 0)
    ...
    def commission_earned(self):
        ...
        # Use prefetched ProjectStaff if available, otherwise query
        if '_prefetched_project_staff' in self.__dict__:
            project_staff = self.__dict__['_prefetched_project_staff']
        else:
            project_staff = ProjectStaff.query.filter_by(
        ...
        if role == 'associate':
            # Rule 1: Associates earn commission on their hours worked
            base_commission = self.hours_worked * hourly_rate * commission_pct

        elif role == 'director':
            # Rule 2: Directors earn commission on their own hours worked
            base_commission = self.hours_worked * hourly_rate * commission_pct

            # Add 2% override commission on total project revenue
            total_revenue = session.query(HoursEntry).filter_by(project_id=self.project_id).join(Employee).with_entities(
            ...
            # Rule 3: Others get % of associate revenue in same project
            associate_ids = session.query(ProjectStaff.employee_id).join(Employee).filter(
                ProjectStaff.project_id == self.project_id,
                Employee.role.ilike('associate')
            ).all()
            associate_ids = [id for (id,) in associate_ids]

            total_associate_revenue = session.query(func.sum(
                HoursEntry.hours_billed * (Employee.hourly_rate or 0))
            ).join(Employee).filter(
                HoursEntry.project_id == self.project_id,
                HoursEntry.employee_id.in_(associate_ids)
            ).scalar() or 0.0

            base_commission = total_associate_revenue * commission_pct
